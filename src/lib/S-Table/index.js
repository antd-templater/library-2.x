...
export default {
  ...
  props: {
    ...
    handleData: {
      type: Function,
      default: function (response, Util) {
  ...
  methods: {
    ...
    loadData (pagination = {}, filters = {}, sorter = {}) {
      ...
      Promise.resolve(this.data(parameter))
        .then(response => {
          Promise.resolve(this.handleData(response, Util))
            .then(result => {
    ...
    updateExpand (expandedRowKeys) {
      if (Util.isArray(this.expandedRowKeys) && Util.isArray(expandedRowKeys)) {
    ...
    initColumnField (columns) {
      for (const column of columns) {
        if (!Util.isArray(column.children) || column.children.length === 0) {
          if (
        ...
        if (this.rowSelection) {
          return (props[k] = {
            ...
            onChange: (selectedRowKeys, selectedRows) => {
              this.updateSelect(selectedRowKeys, selectedRows)
              Util.isFunction(this[k].onChange) && this[k].onChange(selectedRowKeys, selectedRows)
            }
          })
        }
        return (props[k] = null)
