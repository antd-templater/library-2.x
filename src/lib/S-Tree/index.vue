      ...
      >
        ...
        <template #treeTitleRoot="item">
          <slot
            ...
            :loaded="item.loaded"
          >
            <div class="s-tree-title-container">
              <slot
                name="titleRootLabel"
                :level="item.level"
                :label="item.label"
...
<script>
import SEllipsis from '../S-Ellipsis/index.vue'
...
export default {
  ...
  watch: {
    ...
    loadKeys: {
      handler () {
        if (Util.isEmptyArray(this.loadKeys)) {
          this.$emit('load', {
  ...
  methods: {
    ...
    renderSwitcher (node) {
      ...
      const isAsyncNode = !isLeafedNode && Util.isFunction(this.loadData)
      ...
      if (this.showLine) {
        if (Util.isNotEmptyArray(node.children) || (isAsyncNode && !isLoadedNode)) {
    ...
    resetNodeConfig (force) {
      ...
      if (Util.isNotEmptyArray(this.checkedNodes)) {
        ...
        while (downHandleNode) {
          if (flatTreeNodes.some(every => every.value === downHandleNode.value)) {
            if (Util.isNotEmptyArray(childTreeNodes[downHandleNode.value])) {
              upHandleNodes.push(
        ...
        // 向上处理
        if (Util.isNotEmptyArray(upHandleNodes)) {
          let tempUpNodes
          let tempUpNode
          ...
          while (tempUpNode) {
            ...
            if (Util.isNotEmptyArray(tempUpNodes)) {
              tempUpNode = tempUpNodes.pop()
            } else if (Util.isNotEmptyArray(upHandleNodes)) {
              upHandleNode = upHandleNodes.pop()
              tempUpNode = upHandleNode
      ...
      // 排序逻辑
      if (Util.isArray(this.expandedKeys)) {
      ...
      if (Util.isArray(this.selectedNodes)) {
      ...
      if (Util.isArray(this.selectedLinkNodes)) {
      ...
      if (Util.isArray(this.checkedNodes)) {
      ...
      if (Util.isArray(this.checkedHalfNodes)) {
      ...
      if (Util.isArray(this.checkedLinkNodes)) {
    ...
    cleanNodeConfig (force) {
      ...
      this.loadKeys.splice(
        ...
        ...this.loadKeys.filter(value =>
          this.flatTreeNodes.some(every => every.value === value && Util.isNotEmptyArray(every.children))
        )
      )
      ...
      // 默认设置 - expanded
      if (Util.isEmptyArray(this.expandedKeys)) {
      ...
      // 默认设置 - selected
      if (Util.isEmptyArray(this.selectedNodes)) {
        if (this.selectable && !this.allowUnSelected) {
          this.selectedNodes.push(...this.flatTreeNodes.filter(item => item.selectable).slice(0, 1))
        }
      }
      ...
      // 默认设置 - checked
      if (Util.isEmptyArray(this.checkedNodes)) {
        if (this.checkable && !this.allowUnChecked) {
          this.checkedNodes.push(...this.flatTreeNodes.filter(item => item.checkable).slice(0, 1))
        }
      }
    ...
    reloadTreeNodes (nodes, parent) {
      if (!Util.isNotEmptyArray(nodes)) {
        if (!Util.isNotEmptyObject(parent)) {
          this.treeNodes = []
      ...
      if (Util.isNotEmptyObject(parent)) {
        ...
        if (Util.isNotEmptyArray(parent.children)) {
    ...
    compileTreeNodes (nodes, parent) {
      ...
      for (const item of nodes) {
        ...
        // selectable
        item.selectable = Util.isFunction(this.allowSelectedLevel)
          ? !!this.allowSelectedLevel(item)
          : item.level >= this.allowSelectedLevel

        // checkable
        item.checkable = Util.isFunction(this.allowCheckedLevel)
    ...
    collapseTreeNodes (expandedKeys) {
      if (Util.isNotEmptyArray(expandedKeys)) {
        for (const value of expandedKeys) {
          ...
          if (this.expandedKeys.includes(value)) {
            ...
            if (this.allowAutoCollapsed) {
              if (Util.isNotEmptyArray(this.expandedKeys) && Util.isNotEmptyObject(selectedNode)) {
      ...
      if (Util.isNotEmptyArray(this.expandedKeys)) {
        this.expandedKeys.sort(
          (a, b) =>
            this.flatTreeNodes.findIndex(node => node.value === a) -
            this.flatTreeNodes.findIndex(node => node.value === b)
    ...
    expandTreeNodes (expandedKeys) {
      if (Util.isNotEmptyArray(expandedKeys)) {
        for (const expandedKey of expandedKeys) {
          ...
          if (
            selectedNode &&
            (Util.isNotEmptyArray(selectedNode.children) || (isAsyncNode && !isLoadedNode))
          ) {
            ...
            if (this.allowAutoExpanded) {
              ...
              while (
                onlyOneChild &&
                Util.isNotEmptyObject(firstChildNode) &&
                Util.isNotEmptyArray(firstChildNode.children) &&
      ...
      if (Util.isNotEmptyArray(this.expandedKeys)) {
        ...
        if (Util.isFunction(this.loadData)) {
          this.doTreeLoad(
    ...
    doTreeAllCollapse (expandedKeys) {
      if (this.treeNodes.length <= 1 && Util.isNotEmptyArray(this.flatTreeNodes)) {
    ...
    doTreeAllExpand (expandedKeys) {
      this.expandedKeys.splice(
        ...
        this.expandedKeys.length,
        ...this.flatTreeNodes
          .filter(every => Util.isNotEmptyArray(every.children))
    ...
    doTreeExpand (expandedKeys) {
      ...
      if (Util.isNotEmptyArray(delExpandedNodes)) {
        this.collapseTreeNodes(delExpandedKeys)
      }

      if (Util.isNotEmptyArray(addExpandedNodes)) {
        if (!this.allowMultiExpanded) {
          ...
          if (Util.isNotEmptyArray(parents)) {
            expandKeys.push(...parents.map(node => node.value))
          }

          if (Util.isNotEmptyArray(childs) || (isAsyncNode && !isLoadedNode)) {
            expandKeys.push(firstValue)
            expandKeys.push(
              ...childs
                .filter(child => addExpandedKeys.includes(child.value))
                .filter(child => Util.isNotEmptyArray(child.children))
                .map(node => node.value)
            )
          }
    ...
    doTreeSelect (selectedKeys) {
      if (!this.selectable) {
        if (Util.isNotEmptyArray(selectedKeys)) {
      ...
      if (Util.isNotEmptyObject(newFirstSelectedNode)) {
      ...
      if (!Util.isNotEmptyObject(newFirstSelectedNode)) {
        !allowUnSelected && Util.isNotEmptyObject(oldFirstSelectedNode)
      ...
      if (!Util.isNotEmptyArray(this.selectedNodes)) {
        if (Util.isNotEmptyObject(oldFirstSelectedNode) && this.allowAutoCollapsed) {
      ...
      if (!nowSomeSelectedNode && Util.isNotEmptyObject(oldFirstSelectedNode)) {
        delSelectedNodes = !this.selectedNodes.includes(oldFirstSelectedNode)
          ? [oldFirstSelectedNode]
          : []
      }

      if (!nowSomeSelectedNode && Util.isNotEmptyObject(newFirstSelectedNode)) {
    ...
    doTreeCheck (checkedKeys) {
      ...
      if (Util.isNotEmptyArray(delCheckedNodes)) {
      ...
      if (Util.isNotEmptyArray(addCheckedNodes)) {
    ...
    doTreeLoad (loadKeys) {
      ...
      if (Util.isNotEmptyArray(loadKeys)) {
        for (const loadKey of loadKeys) {
          ...
          promises.push(
            Promise.resolve(this.loadData(loadNode))
              .then(nodes => {
                if (Util.isNotEmptyArray(nodes) && this.flatTreeNodes.some(findNode)) {
                  if (this.allowAutoExpanded) {
